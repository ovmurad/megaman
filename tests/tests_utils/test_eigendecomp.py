...
from src.megaman.utils.eigendecomp import (eigen_decomposition, null_space, EIGEN_SOLVERS)

SPD_SOLVERS = EIGEN_SOLVERS
NON_SPD_SOLVERS = ['auto', 'dense', 'arpack']
SOLVER_KWDS_DICT = {'auto':None,
                    'dense':{'driver': 'evr', 'type':1},
    ...
    for largest in [True, False]:
        ...
        for i in range(len(solvers_to_test)):
            for j in range(i+1, len(solvers_to_test)):
                ...
                np.testing.assert_array_almost_equal(Lambdas[solvers_to_test[i]],
