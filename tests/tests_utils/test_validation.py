...
from src.megaman.utils.testing import (assert_no_warnings, assert_warns,
                                       ignore_warnings, assert_raise_message)
from src.megaman.utils.validation import check_array, check_symmetric, DataConversionWarning
...
def test_check_array():
    ...
    # force_all_finite
    X_inf = np.arange(4).reshape(2, 2).astype(float)
    X_inf[0, 0] = np.inf
    with pytest.raises(ValueError):
        check_array(X_inf)
    check_array(X_inf, force_all_finite=False)  # no raise
    # nan check
    X_nan = np.arange(4).reshape(2, 2).astype(float)
    X_nan[0, 0] = np.nan
    with pytest.raises(ValueError):
        check_array(X_nan)
    check_array(X_inf, force_all_finite=False)  # no raise
    ...
    dtypes = [np.int32, int, float, np.float32, None, bool]
    orders = ['C', 'F', None]
    copys = [True, False]
...
def test_check_array_min_samples_and_features_messages():
    ...
    X_checked = assert_warns(DeprecationWarning, check_array, [42],
                             ensure_2d=True)
    np.testing.assert_array_equal(np.array([[42]]), X_checked)
...
def test_check_symmetric():
    ...
    for arr_format, arr in test_arrays.items():
        ...
        if sp.issparse(output):
            ...
            np.testing.assert_array_equal(output.toarray(), arr_sym)
        else:
            np.testing.assert_array_equal(output, arr_sym)
