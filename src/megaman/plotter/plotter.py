...
from .covar_plotter3 import covar_plotter3d_matplotlib, covar_plotter3d_plotly
from .scatter_3d import scatter_plot3d_matplotlib, scatter_plot3d_plotly
from .utils import (
    _check_backend,
    generate_colors_and_colorscale,
    generate_grid,
    get_colors_array,
    get_top_two_sigma_norm,
    plotly_layout,
)
...
def plot_with_plotly(
    embedding, rieman_metric, nstd=2, color_by_ratio=True, if_ellipse=False
):
    import plotly.graph_objs as go
    from plotly.offline import iplot

    sigma_norms = get_top_two_sigma_norm(rieman_metric, color_by_ratio)
    colors, colorscale = generate_colors_and_colorscale("gist_rainbow", sigma_norms)
    scatter_pt = scatter_plot3d_plotly(
        embedding, coloring=sigma_norms, colorscale=colorscale
    )
    index = generate_grid(embedding.shape[0])

    if if_ellipse:
        ellipses_pt = covar_plotter3d_plotly(embedding, rieman_metric, index, colors)
...
def plot_with_matplotlib(
    ...
    index = generate_grid(embedding.shape[0])
    if if_ellipse:
        ax = covar_plotter3d_matplotlib(embedding, rieman_metric, index, ax, colors)
    plt.show()
